...
class GbBlogParse:
    ...
    def compile_comments(self, list_comments, itm):
        dict_comment = itm.get("comment")
        child_comments = []
        for under_itm in dict_comment["children"]:
        ...
        new_dict = {"id": dict_comment["id"],
                    ...
                    "author": {"id": dict_comment["user"]["id"],
                               ...
                               "url": dict_comment["user"]["url"]},
                    "comments": child_comments}
        list_comments.append(new_dict)

    def parse_comments(self, url, id_post):
        url_path = "/api/v2/comments?commentable_type=Post&commentable_id=" + id_post
        response = self._get_response(urljoin(url, url_path))
        list_comments = []
        for itm in response.json():
            self.compile_comments(list_comments, itm)
        return list_comments

    def parse_post(self, url, soup):
        title_tag = soup.find("h1", attrs={"class": "blogpost-title"})
        image_href = soup.find("img").attrs.get("src")
        datetime_post = soup.find("time", attrs={"itemprop": "datePublished"}).get("datetime")
        author_tag = soup.find(attrs={"class": "row m-t"})
        author_href = author_tag.find("a").get("href")
        author_name = author_tag.find(attrs={"text-lg"})
        list_comments = self.parse_comments(url, soup.find("comments").get("commentable-id"))

        data = {
            "url": url,
            "title": title_tag.text,
            "image": image_href,
            "date": dt.datetime.strptime(datetime_post, "%Y-%m-%dT%H:%M:%S%z"),
